...
fun getImporterInstanceList(): MutableList<BaseImporter> {
    ...
    if (!assimp.ASSIMP_BUILD_NO_MD2_IMPORTER) {
//        res.add(Md2Importer())
    }

    if (!assimp.ASSIMP_BUILD_NO_STL_IMPORTER)
        res.add(STLImporter())

    if (!assimp.ASSIMP_BUILD_NO_OBJ_IMPORTER)
        res.add(ObjFileImporter())

    if (!assimp.ASSIMP_BUILD_NO_PLY_IMPORTER)
        res.add(PlyLoader())

    if(!assimp.ASSIMP_BUILD_NO_COLLADA_IMPORTER)
