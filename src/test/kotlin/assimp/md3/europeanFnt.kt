...
object europeanFnt {
    ...
    operator fun invoke(fileName: String, shaderName: String) {
	    ...
	    val baseFileBuffer = fileName.loadFile()
	    val shaderBuffer = shaderName.loadFile()

	    Importer().readFilesFromMemory(fileName,fileName to baseFileBuffer, shaderName to shaderBuffer)
			    ?.verify() ?: fail("could not load $fileName from memory")
    }

	private fun AiScene.verify() {
